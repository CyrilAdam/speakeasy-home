// cocktails-data.js — Bar App Data
...
window.APP_DATA = (function () {
  ...
  const bottles = [
    ...
    // ── Pantry / mixers (always available) ────────────────────────────
  ...
  // ── Compute makeable status ──────────────────────────────────────────
  const bottleMap = Object.fromEntries(bottles.map(b => [b.id, b]));

  cocktails.forEach(c => {
    const missing = c.ingredients.filter(ing => {
      const bottle = bottleMap[ing.id];
      return !bottle || !bottle.owned;
    });
    c.canMake      = missing.length === 0;
    c.missingCount = missing.length;
    c.missingIngredients = missing.map(ing => bottleMap[ing.id]?.name || ing.id);
  });

  return { bottles, cocktails, bottleMap };
})();
