...
export function createCocktailsRouter(db: DatabaseSync) {
  const app = new Hono();
  ...
  app.post('/', async (c) => {
    ...
    withTransaction(() => {
      ...
      (body.ingredients ?? []).forEach((ing, i) => insertIng.run(body.id, ing.id, ing.amount, i));
    });
  ...
  app.patch('/:id', async (c) => {
    const id = c.req.param('id');
    if (!db.prepare('SELECT id FROM cocktails WHERE id = ?').get(id)) {
      return c.json({ error: 'Cocktail introuvable' }, 404);
    }
    ...
    withTransaction(() => {
      ...
      if (body.ingredients !== undefined) {
        ...
        body.ingredients.forEach((ing, i) => insertIng.run(id, ing.id, ing.amount, i));
      }
    });
